  ...
  const handlePetalClick = (e: React.MouseEvent, id: number) => {
    ...
    setPetals(prev => prev.map(p => {
      if (p.id === id) {
        ...
        // We must slightly change ID or key to force React/CSS to re-paint the animation
        // But changing ID messes up the map key.
        // Strategy: Just reset properties and hope CSS handles it, or toggle a key.
        // A simpler way for CSS animation reset:
